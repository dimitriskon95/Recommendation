...
void ClusterEuclidean::set_Pnumber(char *line)
{
...
//-------------------------------------------------------------------MENU FUNCTION-----------------------------------------------------------------------------
void ClusterEuclidean::ClusterEuclidean_Menu(ifstream& infile1, ifstream& infile2, ofstream& outfile)
{
	outfile << "<Euclidean Clustering>" << '\n';
	...
	Euclidean_User_Reader(infile1, infile2);
	...
	{	
		...
		items_data = Euclidean_Clustering(Clustering_node, outfile);
...
void ClusterEuclidean::Euclidean_User_Reader(std::ifstream& infile, std::ifstream& tmp_file)
{
...
double *ClusterEuclidean::Euclidean_Clustering(Node<double> *node, ofstream& outfile)
{
	cout << "Function Euclidean Clustering was called for user with id: " << node->get_Number() << endl;
	...
	{
		neighbor = Euclidean_close_Neighbor(node, ulist);
		if (neighbor != NULL)
	...
	for(int j=0; j < 5; j++)
	{
		...
		if (index >= 0){
			if (max == node->get_Average() || max == 0)
			{
				...
				do{
					item_number = uniform_distribution(0, dimensions-1);
...
Node<double> *ClusterEuclidean::Euclidean_close_Neighbor(Node<double> *node, List<double> *ulist)
{
...
double ClusterEuclidean::Similarity(Node<double>* user, Node<double>* node)
{
	int dims = dimensions;
...
void ClusterEuclidean::Euclidean_Validation_Menu(std::ifstream& infile, std::ifstream& tmp_file, std::ofstream& outfile, int usersNumber)
{
	...
	{	
		...
		{	
			...
			predict_array = Euclidean_Clustering(Clustering_node, outfile);
		...
		avgValidation += (sum / validation->get_J());
		
		cout << "For subject " << validation_list << " MAE is " << (sum / validation->get_J()) << endl;
	...
	outfile << "<Euclidean Clustering> MAE: " << avgValidation << "\n";
}
